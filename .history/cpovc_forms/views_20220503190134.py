...
def case_transfer(request, id):
    try:
        ...
        if request.method == 'POST':
            ...
            organization = str(request.POST.getlist("ORG_UNIT")[index])
            ...
            for hhmember in hhmembers:
                ...
                print(index, id, event_id, house_hold,date_of_transfer,
                      reason, date_follow_up, '@reck')

                OVCCareTransfer(
                    person_id=person_id,
                    event_id=event_id,
                    household=house_hold,
                    rec_organization_id=organization,
                    date_of_event=date_of_transfer,
                    reason=reason,
...
def edit_transfer(request, id):
    ...
    transfer = OVCCareTransfer.objects.get(init_data)
    child = RegPerson.objects.get(id=id)
