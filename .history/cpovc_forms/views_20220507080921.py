...
def edit_transfer(request, id):
    ...
    for transferedCase in case_transfers:
        ...
        if request.method == 'POST':
            print(request.POST)
            pdb.set_trace()
            editOrg = OVCCareTransfer.objects.filter(rec_organization_id=organization_id).update(rec_organization_id=organization_id, is_void=True)
