...
from .models import (
    ...
    OVCCareBenchmarkScore, OVCMonitoring, OVCHouseholdDemographics, OVCHivStatus, OVCHIVManagement, OVCHIVRiskScreening, OVCCareTransfer)
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            for hhmember in hhmembers:
                ...
                OVCRegistration.objects.filter(person_id=id).update(child_cbo=organization_id, is_void=True)
                RegPersonsOrgUnits.objects.filter(person_id=id).update(org_unit_id=organization_id, is_void=True)
            ...
            for i in range(0,len(transferedMembers)):
                if transferedMembers[i].is_void == False:
...
def edit_transfer(request, id):
    ...
    for transferedCase in case_transfers:
        ...
        if request.method == 'POST':
            ...
            editOrg = OVCCareTransfer.objects.filter(currentOrg=organization_id).update(rec_organization_id=organization_id, is_void=True)
            
            OVCCareTransfer.save(update_fields=['rec_organization_id=organization_id, is_void=Tru'])
            pdb.set_trace()
            return redirect('forms/case_transfer.html')
...
def delete_transfer(request):
    ...
    case_transfer = OVCCareTransfer.objects.filter(transfer_id=transfer_id).update(is_void=True)
    object = OVCCareTransfer.objects.get(transfer_id=transfer_id)
    deleted = object.is_void
    # pdb.set_trace()
    if deleted == True:
        data = {
            'deleted': True
