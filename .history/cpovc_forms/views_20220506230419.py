...
def delete_transfer(request):
    ...
    case_transfer = OVCCareTransfer.objects.filter(transfer_id=transfer_id)
    if case_transfer:
        object = OVCCareTransfer.objects.get(transfer_id=transfer_id)
        update_is_void = OVCCareTransfer.objects.filter(transfer_id=transfer_id).update(is_void=True)
