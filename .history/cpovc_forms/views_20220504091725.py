...
from cpovc_forms.forms import (
    ...
    HIV_MANAGEMENT_ARV_THERAPY_FORM, HIV_MANAGEMENT_VISITATION_FORM, DREAMS_FORM, CaseTransferForm, EditCaseTransferForm)
...
def case_transfer(request, id):
    try:
        ...
        transferedMembers = OVCCareTransfer.objects.get(id=OVCHHMembers.objects.get(person=child).house_hold_id)
        pdb.set_trace()
...
def edit_transfer(request, id):
    import pdb 
    EditcaseTransferForm = EditCaseTransferForm()
    transfer = OVCCareTransfer.objects.get(transfer_id='ede240ca-d865-4366-9135-1694f2d6941d')
    child = RegPerson.objects.get(id=id)
