...
def case_transfer(request, id):
    try:
        ...
        if request.method == 'POST':
            ...
            organization_id =  OVCCareTransfer.objects.filter(person=id).rec_organization_id  
            transferedMembers = OVCCareTransfer.objects.filter(person=id).rec_organization_id 
            # for transferedMember in transferedMembers:
            #     # print(transferedMember.rec_organization.id)
                # RegOrgUnit.objects.get(
                #     pk=int(organizationUnitID))
