...
def case_transfer(request, id):
    try:
        ...
        # currentOrganization = OVCRegistration.objects.get(is_void=False, person_id=id).child_cbo.id
        # pdb.set_trace()
        
        # Get transferred Members
        
        # organization = transferedMembers.rec_organization_id.get()
        # pdb.set_trace() 
        ...
        if request.method == 'POST':
            ...
            for hhmember in hhmembers:
                ...
                print(index, id, event_id, house_hold,date_of_transfer,
                      reason, date_follow_up, '@k')

                OVCCareTransfer(
                    person_id=hhmember,
                    event_id=event_id,
                    household=house_hold,
                    rec_organization_id=organization_id,
                    date_of_event=date_of_transfer,
                    reason=reason,
                    date_follow_up=date_follow_up,
                ).save()
                # OVCRegistration.objects.get(is_void=False, person_id=id).child_cbo.id
                OVCRegistration.objects.filter(person_id=id).update(child_cbo=organization_id)
                
                msg = 'Transfer inititiated successful'
                
                
                print("############################<<<<<<<<!!!>>>>>>>>>>####################")
                # print(OVCRegistration.objects.get(is_void=False, person_id=id).child_cbo.id)
            ...
            # pdb.set_trace()
            # for transferedMember in transferedMembers:
            #     # print(transferedMember.rec_organization.id)
            
            # org_name = RegOrgUnit.objects.get(pk= OVCCareTransfer.objects.get(person=id).rec_organization_id)
            # pdb.set_trace()
            # RegOrgUnit.objects.get(pk=int(organization_id))
            ...
            return render(
                ...
                context={
                    ...
                    'hhid':hhid,
                    'hhmembers':hhmembers,
                    'transferedMembers':transferedMembers,
                    }
            )
    except Exception as e:
        raise e
...
def edit_transfer(request, id):
    import pdb 
    # Load all person based on event_id
    case_transfers = OVCCareTransfer.objects.filter(event_id=id)
    caseTransferForm = CaseTransferForm()
    transferOrganization = case_transfers
    # pdb.set_trace()
    
    # Get current Organization
    OvcOrg = OVCRegistration.objects.all()
    
    # Update person org unit(RegPerson table)
    transferedMembers = OVCCareTransfer.objects.all() 
    # organization_id =  OVCCareTransfer.objects.filter(person=60)
    # pdb.set_trace()
    for memberTransfered in transferedMembers:
        # pdb.set_trace()
        reason = memberTransfered.reason
        follow_up = memberTransfered.date_follow_up
        comment = memberTransfered.comment
      
        # for org_id in organization_id:
            
        # Get organization name
        # transferedMembers[0].rec_organization
        # Get org id
        # transferedMembers[0].rec_organization.id
        
        
        # Reverse the reg person unit to previous OVC registration on delete
        # Void existing  & insert row Reg person units
        # Update OVC registration 
    
        return render(
                    request,
                    template_name=
                        'forms/edit_transfer.html',
                    context={
                        'case_transfers':case_transfers,
                        'case_transfer_form':caseTransferForm,
                        'reason':reason,
                        'follow_up': date_follow_up,
                        'comment': comment
                    }
                )
