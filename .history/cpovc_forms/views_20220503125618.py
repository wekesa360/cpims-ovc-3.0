...
def edit_transfer(request, id):
    ...
    hhid = hhold.house_hold_id
    care_giver = RegPerson.objects.get(id=OVCRegistration.objects.get(person=child).caretaker_id)
    householdmembers = OVCHHMembers.objects.filter( is_void=False, house_hold_id=hhid)
    ...
    for members in householdmembers:
        f_name = member.first_name
        l_name = member.surname
        return fname, la_name
