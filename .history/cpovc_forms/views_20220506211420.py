...
def delete_transfer(request, id, btn_event_pk):
    
    transfer_id = uuid.UUID(btn_event_pk)
    case_transfer = OVCCareTransfer.objects.filter(event_id=id)
    if case_transfer:
        OVCCareTransfer.objects.filter(event_id=id).delete()
