...
def case_transfer(request, id):
    ...
    if request.method == 'POST':
        event_id = 'FCSI'
        date__transfer = request.POST.get('date_of_transfer')
        if date_transfer:
           date_transfer = convert_date(date_of_transfer)
           
           
        # event_id = OVCCareEvents(
        #     event_name = ""    
        #     ).save()
        ...
        ovctransferevent = OVCCareEvents(
            ...
            event_counter=event_counter,
            event_score=0,
            date_of_event=date_transfer,
            created_by=request.user.id,
            person=RegPerson.objects.get(pk=int(id)),
            # house_hold = OVCHouseHold.objects.get(id=OVCHHMembers.objects.get(person=child).house_hold_id)
        )
        ovctransferevent.save
        ...
        OVCCareTransfer(
            ...
            date_transfer = request.POST.get("transfer_date")
        ).save()
