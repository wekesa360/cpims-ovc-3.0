...
def case_transfer(request, id):
    try:
        ...
        house_hold = OVCHouseHold.objects.get(
            id=OVCHHMembers.objects.get(person=person_id).house_hold_id)
        pdb.set_trace()
        siblings = RegPersonsSiblings.objects.select_related().filter(
            child_person=id, is_void=False, date_delinked=None)
        hhold = OVCHHMembers.objects.get(
        ...
        if request.method == 'POST':
            ...
            transferedMembers = OVCCareTransfer.objects.filter(person=id) 
...
def edit_transfer(request, id):
    import pdb 
    transfer = OVCCareTransfer.objects.get(transfer_id=id)
    child = RegPerson.objects.get(id=id)
    ...
    # pdb.set_trace()
    # renderedHouseholdInf = {
        
    # }
    transfer_get = {
        't_reason': transfer.reason,
        't_organization': transfer.rec_organization_id,
        'tperson': transfer.person_id,
    }
    ...
    return render(
                ...
                context={
                    'hhmembers':transfer_get,
                    'edit_transer':edit_transfer,
                }
            )
