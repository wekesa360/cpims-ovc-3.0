...
class OVCCareEvents(models.Model):
    ...
    # app_user = models.ForeignKey(AppUser, default=1)
    person = models.ForeignKey(RegPerson, on_delete=models.CASCADE,  null=True)
     = models.ForeignKey(OVCHouseHold, on_delete=models.CASCADE,  null=True)
...
class OVCCareTransfer(models.Model):
    ...
    person = models.ForeignKey(RegPerson, on_delete=models.CASCADE)
    # caregiver = models.ForeignKey(RegPerson, on_delete=models.CASCADE)          # issues on migrating
    # household = models.ForeignKey(OVCHouseHold, on_delete=models.CASCADE)
    reason=models.CharField(max_length=10, null=False)
    # organization=models.ForeignKey(RegOrgUnit, on_delete=models.CASCADE)        # issues on migrating
    # case_manager= models.ForeignKey(RegPerson, on_delete=models.CASCADE)        # issues on migrating
    # rec_organization=models.ForeignKey(RegOrgUnit, on_delete=models.CASCADE)
    # rcase_manager= models.ForeignKey(RegPerson, on_delete=models.CASCADE)       # issues on migrating
    # date_of_event = models.DateField()
    date_follow_up=models.DateField()
    date_transfer=models.DateField()
    # form = models.ForeignKey(OVCCareForms, on_delete=models.CASCADE)            # issues on migrating
    is_void = models.BooleanField(default=False)
    # event = models.ForeignKey(OVCCareEvents, on_delete=models.CASCADE)          # issues on migrating
    timestamp_created = models.DateTimeField(default=timezone.now)
    timestamp_updated = models.DateTimeField(auto_now=True)
