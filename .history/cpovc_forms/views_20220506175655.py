    ...
    try:
        ...
        if request.method == 'POST':
            ...
            for hhmember in hhmembers:
                ...
                # Update child_CBO on OVCReg table & # Update person org unit on reg-person-org-unit table
                OVCRegistration.objects.filter(person_id=id).update(child_cbo=organization_id)
                
                RegPersonsOrgUnits.objects.filter(person_id=id).update(org_unit_id=organization_id)
