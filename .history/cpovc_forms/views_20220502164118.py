...
from cpovc_registry.functions import save_audit_trail
...
from cpovc_main.functions import get_list_of_org_units
...
def case_transfer(request, id):
    ...
    if request.method == 'POST':
        ...
        for hhmember in hhmembers:
            ...
            reason = request.POST.get("REASON")[index],
            date_follow_up = request.POST.get("FOLLOW_UP_DATE")[index],
            date_of_transfer = request.POST.get("TRANSFER_DATE")[index]
            print (index, reason, date_follow_up, date_of_transefer)
        
        event_counter = OVCCareEvents.objects.filter(
                event_type_id=event_id, person=id, is_void=False).count()
        
        import pdb
        ovccareevent = OVCCareEvents({{ case_transfer_form.TRANSFER_DATE }}
        ...
        OVCCareTransfer(
            person = RegPerson.objects.get(id=id),
            event_id = 'b7ea884e-c933-11ec-89a4-e1fe85340965',
            household = OVCHouseHold.objects.get(id=OVCHHMembers.objects.get(person=child).house_hold_id),
            rec_organization=  RegOrgUnit.objects.order_by('org_unit_name')[0],
            date_of_event = date_of_transfer,
            reason = request.POST.get("REASON"),
            date_follow_up = request.POST.get("FOLLOW_UP_DATE"),
            date_of_transfer = request.POST.get("TRANSFER_DATE")
        ).save()
        ...
        if request.POST.get("saveTransfer"):
            for member in hhmembers:
                if request.POST.get("c" + str(member.id) == "clicked"):
                    member.complete == True
                else:
                    member.complete == False
                member.save()
                
        elif request.POST.get("newTransfer"):
            transfer = request.POST.get("newTranser")
